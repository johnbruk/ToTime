...
const EXTRA=`
  engagements:[{id:'e1',user_id:'u1',client_id:'c1',project_id:'p1',code:'SO-EQU-2026-001',year:2026,seq:1,name:'Incarico',status:'active',invoice_reference:'LI_202601_Nome_Cognome',engagement_letter:'LI_202601'}],
  ...
  wbs_items:[
    ...
    {id:'w30',user_id:'u1',engagement_id:'e1',activity_code:'30',code:'SO-EQU-2026-001-30',name:'Interne',kind:'internal',billable:false,status:'active',sort_order:30}],
...
const srv=http.createServer((q,s)=>{let p=decodeURIComponent(q.url.split('?')[0]);if(p==='/')p='/index.html';
  fs.readFile(path.join(ROOT,p),(e,b)=>{if(e){s.writeHead(404);s.end('x');return;}
    if(p.endsWith('mock.html')){let h=b.toString();
      ...
      h=h.replace("{id:'p1',client_id:'c1',name:'Beta',active:true}","{id:'p1',client_id:'c1',short_code:'EQU',code:'SO-EQU',name:'Beta',end_client_name:'EQUANS',billing_unit:'day',status:'active',active:true}");
...
ok(L&&L.snapshot_project_code==='SO-EQU','e lo snapshot del codice progetto',L&&L.snapshot_project_code);
...
ok(/SO-EQU/.test(re[0]||''),'ma il codice progetto c\'è',re[0]);
