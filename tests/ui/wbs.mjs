// Gerarchia Cliente > Commessa > Progetto > WBS nei moduli di
...
const EXTRA=`
  engagements:[{id:'e1',user_id:'u1',client_id:'c1',code:'SO-2026-001',year:2026,seq:1,name:'Incarico 2026',status:'active',engagement_letter:'LI_202601',invoice_reference:'LI_202601_Nome_Cognome'}],
  ...
  wbs_items:[
    ...
    {id:'w90',user_id:'u1',project_id:'p1',activity_code:'90',code:'SO-2026-001-EQU-90',name:'Trasferte',kind:'travel',billable:false,status:'closed',sort_order:90}],
    ...
    fs.readFile(path.join(ROOT,p),(e,b)=>{if(e){s.writeHead(404);s.end('x');return;}
      if(p.endsWith('mock.html')){let h=b.toString();
        if(conGerarchia){
          ...
          h=h.replace("{id:'p1',client_id:'c1',name:'Beta',active:true}","{id:'p1',client_id:'c1',engagement_id:'e1',short_code:'EQU',code:'SO-2026-001-EQU',name:'Beta',status:'active',active:true}");
        }
        b=Buffer.from(h);}
...
ok((await opts('engagement_id')).includes('e1'),'la commessa e\' selezionabile');
ok((await opts('hier_project_id')).length===0,'il progetto resta vuoto finche\' non si sceglie la commessa');
await pg.selectOption('[name="engagement_id"]','e1');await pg.waitForTimeout(200);
ok((await opts('hier_project_id')).includes('p1'),'scelta la commessa, compaiono i suoi progetti');
ok((await opts('wbs_id')).length===0,'la WBS resta vuota finche\' non si sceglie il progetto');
await pg.selectOption('[name="hier_project_id"]','p1');await pg.waitForTimeout(200);
const wbs=await opts('wbs_id');
ok(wbs.includes('w10')&&wbs.includes('w20'),'scelto il progetto, compaiono le sue WBS',wbs.join(' '));
ok(!wbs.includes('w90'),'la WBS chiusa non e\' selezionabile per nuove registrazioni');
await pg.selectOption('[name="wbs_id"]','w10');await pg.waitForTimeout(200);
ok(/SO-2026-001-EQU-10/.test(await pg.evaluate(()=>document.getElementById('wbsHint')?.textContent||'')),'sotto la WBS si legge il codice completo');
