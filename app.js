...
function homeFatturatoCard(){const yr=currentYear();const cur=annualTotals(yr);const daFatturare=Math.max(0,cur.consuntivato-cur.fatturato);return `<div class="card cardLink" onclick="openAnnualInvoices('issued')" role="button" title="Elenco fatture emesse ${yr}"><b>Il tuo fatturato ${yr} <span class="cardLinkArrow">›</span></b><div class="kpiGrid" style="margin-top:14px"><div><span>Fatturato</span><strong>${fmtEUR(cur.fatturato)}</strong></div><div><span>Da fatturare</span><strong>${fmtEUR(daFatturare)}</strong></div></div></div>`}
...
function billingSummaryCard(){const {year,month}=periodParts();const md=annualMonthData(year);const fatturatoMese=md[month-1]?.fatturato||0;const incassatoMese=md[month-1]?.incassato||0;const at=annualTotals(year);const daFatturareAnno=Math.max(0,at.consuntivato-at.fatturato);return `<div class="card cardLink" onclick="openAnnualInvoices('issued')" role="button" title="Elenco fatture emesse ${year}"><b>Fatturato <span class="cardLinkArrow">›</span></b><div class="kpiGrid" style="margin-top:14px"><div><span>Fatturato mese</span><strong>${fmtEUR(fatturatoMese)}</strong><small>${monthLabel(state.month)}</small></div><div><span>Fatturato anno</span><strong>${fmtEUR(at.fatturato)}</strong><small>${year}</small></div></div><div class="metricLine" style="margin-top:12px">Da fatturare anno <span class="dot">·</span> ${fmtEUR(daFatturareAnno)} <span class="dot">·</span> Incassato mese <span class="dot">·</span> ${fmtEUR(incassatoMese)}</div></div>`}
function billing(){const groups=billingGroupsByClient();const total=groups.reduce((s,g)=>s+g.total,0);return appShell(`<h1>Fatturazione</h1>${monthSelector()}<div class="card"><b>Totale fatturazione mese</b><div class="amount" style="font-size:30px;margin-top:8px">${fmtEUR(total)}</div><div class="sub">Include eventuale rivalsa INPS 4% e marca da bollo se attive.</div></div>${billingSummaryCard()}<div class="list">${groups.map(g=>{const st=headerStatus(g.client_id);return `<div class="row" onclick="openBillingClient('${g.client_id}')"><div></div><div><div class="title">${esc(clientName(g.client_id))}</div><div class="metricLine">${metricLine(g.hours,g.total)}</div><div class="desc">Base ${fmtEUR(g.calc.subtotal)} · Rivalsa ${fmtEUR(g.calc.inpsAmount)} · Bollo ${fmtEUR(g.calc.stampAmount)}</div><span class="tag ${statusClass(st)}">${statusLabel(st)}</span></div><div class="value">›</div></div>`}).join('')||'<div class="empty">Nessuna riga fatturabile.</div>'}</div>`)}
...
function render(){if(state.loading){document.getElementById('app').innerHTML=loadingView();return}if(state.view==='resetPassword'){document.getElementById('app').innerHTML=resetPasswordView();return}if(!session){const authMap={register:registerView,forgotPassword:forgotPasswordView};document.getElementById('app').innerHTML=(authMap[state.view]||loginView)();return}let html='';const map={home,newChoice,dailyForm,dailyEdit,monthlyForm,monthlyEdit,manualForm,manualEdit,expenseForm,expenseEdit,timesheet,summary,billing,billingDetail:billingDetailView,settings,clients,projects,activities,clientEdit,projectEdit,activityEdit,expenseCategories,expenseCategoryEdit,invoiceTemplates,invoiceTemplateEdit,appearance,exportTimesheet,tax,taxPayments,taxPaymentEdit,annualMonths,annualInvoices};html=(map[state.view]||home)();document.getElementById('app').innerHTML=html}
