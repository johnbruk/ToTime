...
const MENU=[
  ...
  {main:'timesheet',ic:'◷',l:'Timesheet',sub:[{v:'newChoice',l:'Nuovo consuntivo'},{v:'calendario',l:'Calendario'},{v:'griglia',l:'Griglia del mese'},{v:'pivot',l:'Analisi consuntivi'},{v:'tmManage',l:'Incarichi continuativi'}]},
...
function newChoice(){return appShell(`<h1>Nuovo consuntivo</h1><p class="sub">Cosa vuoi registrare?</p><div class="actions"><button class="menuBtn" onclick="goForDay('dailyForm')"><span><b>Time &amp; Material</b><br><span class='sub'>Ore effettivamente lavorate, valorizzate secondo tariffa</span></span><span>›</span></button><button class="menuBtn" onclick="goForDay('tmForm')"><span><b>Impegno continuativo</b><br><span class='sub'>Tutti i giorni lavorativi compresi nel periodo selezionato</span></span><span>›</span></button><button class="menuBtn" onclick="goForDay('manualForm')"><span><b>Compenso forfettario</b><br><span class='sub'>Importo fisso, indipendente da ore e giornate</span></span><span>›</span></button><button class="menuBtn" onclick="goForDay('expenseForm')"><span><b>Rimborso spese</b><br><span class='sub'>Trasferte, chilometri, hotel, pasti e parcheggi</span></span><span>›</span></button></div>`)}
...
function tmManage(){const batches=tmBatches();return appShell(`<h1>Periodi Time &amp; Material</h1><p class="sub">Incarichi T&amp;M generati. Elimina un intero periodo in un colpo solo, oppure elimina i singoli giorni dal timesheet.</p>${batches.length?`<div class="list">${batches.map((b,i)=>`<div class="row"><div></div><div><div class="title">${esc(clientName(b.client_id))}${b.project_id?' / '+esc(projectName(b.project_id)):''}${b.planned?' <span class="tag blue">'+b.planned+' pianif.</span>':''}</div><div class="desc">${esc(activityName(b.activity_id)||'')}${b.desc?' · '+esc(b.desc):''}</div><div class="desc">Dal ${fmtDMY(b.start)} al ${fmtDMY(b.end)} · ${b.days} giorni · ${fmtNum(b.hoursSum,1)} h totali · ${fmtNum(b.hours,2)} h/giorno</div><button class="secondary danger" style="margin-top:10px" onclick="deleteTMBatch(${i})">Elimina intero periodo</button></div><div class="value"></div></div>`).join('')}</div>`:emptyState('Nessun incarico continuativo generato.','+ Crea un incarico continuativo',"go('tmForm')")}<div class="actions"><button type="button" class="secondary" onclick="go('timesheet')">Torna al timesheet</button></div>`);}
...
function griglia(){
  ...
  return appShell(`<h1>Griglia del mese</h1>
...
function taxScheduleCard(year,mode,title,desc){
  ...
  return `<div class="card"><b>${title}</b><div class="desc" style="margin-top:2px">${desc}</div><div class="kpiGrid three" style="margin-top:14px"><div><span>Base di calcolo</span><strong>${fmtEUR(d.base)}</strong></div><div><span>Reddito forfettario</span><strong>${fmtEUR(d.forfait)}</strong></div><div><span>Totale da versare</span><strong>${fmtEUR(r.total)}</strong></div></div><div class="metricLine" style="margin-top:8px">${d.parts.map(p=>esc(p[0])+' '+fmtEUR(p[1])).join(' <span class="dot">·</span> ')}</div>${r.items.length?refs.map(rf=>{const its=r.items.filter(i=>Number(i.ref)===rf);const sub=its.reduce((s,i)=>s+i.amount,0);return `<div class="refGroup"><div class="refHead"><b>Riferimento ${rf}</b><span>${fmtEUR(sub)}</span></div><div class="list" style="box-shadow:none;margin:0">${its.map(it=>`<div class="row"><div class="date">${dateIT(it.date)}<br><span style="font-size:11px">${String(it.date).slice(0,4)}</span></div><div><div class="title">${esc(it.label)}</div><div class="desc"><span class="tag ${kindClass[it.kind]||'gray'}">Prevista</span> scadenza ${dateIT(it.date)}/${String(it.date).slice(0,4)}</div></div><div class="value">${fmtEUR(it.amount)}</div></div>`).join('')}</div></div>`}).join(''):'<div class="empty">Nessuna scadenza prevista.</div>'}</div>`;
}
...
function previsioneRicaviCard(){const year=currentYear();const f=forecastCalc(year);if(f.pianificato<=0&&f.daFatturare<=0)return '';return `<div class="card"><b>Previsione ricavi ${year}</b><div class="desc" style="margin-top:2px">Consuntivato maturato + giorni pianificati futuri.</div><div class="kpiGrid three" style="margin-top:14px"><div><span>Consuntivato</span><strong>${fmtEUR(f.consuntivato)}</strong></div><div><span>Pianificato</span><strong>${fmtEUR(f.pianificato)}</strong></div><div><span>Previsione ricavi</span><strong>${fmtEUR(f.ricavi)}</strong></div></div><div class="metricLine" style="margin-top:10px">Già fatturato ${fmtEUR(f.fatturatoBase)} <span class="dot">·</span> Da fatturare ${fmtEUR(f.daFatturare)} <span class="dot">·</span> <span class="tag blue">Pianificato</span> ${fmtEUR(f.pianificato)}</div></div>`;}
...
<div class="card"><b>Margine operativo ${year}</b><div class="amount" style="font-size:30px;margin-top:8px">${fmtEUR(margine)}</div><div class="desc">Ricavi − costi. Rimborsi e bolli si compensano: resta compensi + rivalsa INPS − spese a mio carico.</div></div>
...
<div class="card"><b>Utile netto stimato ${year}</b><div class="amount" style="font-size:32px;margin-top:8px">${fmtEUR(utileNetto)}</div><div class="desc">Margine operativo − imposte − contributi. Stima gestionale, non sostituisce il commercialista. Dettaglio e proiezione in Tassazione ›</div></div>${previsioneBilancioCard()}
